...
def train_model(model, batch_size, epochs,x, model_name,intensity_seq):
        
    training_gen = generate_batch_augment(train_df,batch_size,intensity_seq,True)
    val_gen = generate_batch_augment(val_df,batch_size,intensity_seq, False)

    csv_logger = CSVLogger('training' + model_name + '.log')

    callbacks = [EarlyStopping(monitor='val_dice_coef', patience=15, verbose=1, min_delta=1e-6, mode = 'max'),
                 keras.callbacks.ReduceLROnPlateau(monitor='val_dice_coef', factor=0.5,patience=3, verbose=1, mode='max', cooldown=0, min_lr=1e-8),
                 ModelCheckpoint(model_name + '.h5', monitor='val_dice_coef', verbose=1, save_weights_only = False, save_best_only=True, mode='max'),
                 csv_logger]
...
if __name__ == "__main__":
    ...
    trainpath = '/home/tianlei/Downloads/lung_data/train_data/'#Please enter your complete datapath of the folder containing all the training data in .png format'
    valpath = '/home/tianlei/Downloads/lung_data/val_data/'#'Please enter your complete datapath of the folder containing all the validation data in .png format'
    ...
    train_model(model, batch_size, Epochs, train_df, weightname,intensity_seq)